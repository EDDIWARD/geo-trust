...
def _load_documents() -> list[dict]:
    database_payload = mock_repository.load_rag_documents()
...
def _load_cards() -> list[dict]:
    database_payload = mock_repository.load_rag_cards()
    if database_payload is not None:
        return database_payload
...
def _load_chunks() -> list[dict]:
    database_payload = mock_repository.load_rag_chunks()
    if database_payload is not None:
        return database_payload
    return _load_jsonl(CORPUS_DIR / "chunks.jsonl")


@lru_cache(maxsize=1)
def _load_insights() -> list[dict]:
    database_payload = mock_repository.load_rag_insights()
    if database_payload is not None:
        return database_payload
    return _load_jsonl(CORPUS_DIR / "insights.jsonl")
