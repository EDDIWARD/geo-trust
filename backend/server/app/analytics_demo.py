...
def _build_global_market_heat(products: list[dict]) -> list[DemoMarketInsight]:
    ...
    for product in products:
        for market in product["markets"]:
            city = market["city"]
            city_validation = 0.72 * _city_market_score(product, market) + 0.28 * _global_city_strategic_score(city)
...
def _global_city_strategic_score(city: str) -> float:
    profile = CITIES[city]
    return _clamp(
        0.32 * profile["premium"]
        + 0.24 * profile["gift"]
        + 0.2 * profile["culture"]
        + 0.16 * profile["tea"]
        + 0.08 * profile["beverage"]
    )
...
def _finalize_market_insights(rows: list[dict], scope: str) -> list[DemoMarketInsight]:
    ...
    for index, row in enumerate(ordered):
        stretched_score = _stretch_market_score(row["score"], top_score, bottom_score, scope)
        tier, observation = _market_observation(
            score=stretched_score,
            rank=index,
            total=len(ordered),
            top_score=_stretch_market_score(top_score, top_score, bottom_score, scope),
            spread=max(_stretch_market_score(top_score, top_score, bottom_score, scope) - _stretch_market_score(bottom_score, top_score, bottom_score, scope), 0.01),
            scope=scope,
        )
        results.append(
            DemoMarketInsight(
                ...
                opportunity_score=round(stretched_score * 100, 2),
...
def _stretch_market_score(score: float, top_score: float, bottom_score: float, scope: str) -> float:
    spread = max(top_score - bottom_score, 0.01)
    normalized = (score - bottom_score) / spread
    gamma = 0.78 if scope == "global" else 0.72
    floor = 0.34 if scope == "global" else 0.28
    ceiling = 0.96 if scope == "global" else 0.98
    shaped = normalized**gamma
    return _clamp(floor + shaped * (ceiling - floor))
