...
def initialize_database(settings: Settings) -> None:
    ...
    (settings.static_dir / "product-media").mkdir(parents=True, exist_ok=True)
...
def _seed_regions_if_needed(connection: sqlite3.Connection, sample_regions_path: Path) -> None:
    if not sample_regions_path.exists():
        return

    region_items: list[dict[str, Any]] = json.loads(sample_regions_path.read_text(encoding="utf-8"))
    existing_codes = {
        row["code"]
        for row in connection.execute("SELECT code FROM regions").fetchall()
    }
