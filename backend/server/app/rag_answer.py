...
def _split_block_items(text: str) -> list[str]:
    ...
    for raw_line in text.splitlines():
        ...
        if not line:
            continue
        if re.fullmatch(r"\[[a-z_]{3,}\]?", line, flags=re.IGNORECASE):
            continue
...
def _clean_block_text(text: str) -> str:
    ...
    cleaned = re.sub(
        r"\[(executive_summary|core_judgement|evidence_findings|strategy_actions|pricing_packaging_advice|channel_advice|origin_trust_advice|risk_warning)\]?",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
    cleaned = re.sub(r"\[[a-z_]{3,}\]?", "", cleaned, flags=re.IGNORECASE)
...
def _refine_analysis(report, analysis: DemoLlmStrategyAnalysis, card_hits: list[dict], insight_hits: list[str]) -> DemoLlmStrategyAnalysis:
    ...
    top_city = report.market_insights[0].city if report.market_insights else "核心样板城市"
    ...
    if mode == "under":
        fallback_judgement = (
            f"{product.product_name} 当前属于“高内在价值、低市场呈现”的低估潜力型，核心任务是把真实价值表达清楚并在目标城市验证承接。"
        )
        fallback_summary = (
            f"{product.product_name} 当前内在价值 {intrinsic:.1f} 分，显著高于呈现价值 {presented:.1f} 分，"
            f"建议先补强产地证据与消费场景表达，再在 {top_city}、{second_city} 进行小范围验证。"
            f"现阶段激活率 {activation:.1f}%、参与率 {participation:.1f}% 可作为首轮复盘指标。{evidence_hint}"
        )
        fallback_actions = [
            f"明确 {product.product_name} 的主卖点与使用场景，围绕 {product.category} 和 {product.price_band} 做一致化表达。",
            origin_action,
            f"在 {top_city}、{second_city} 以 {product.channel} 做小批量投放，第三城市优先考虑 {third_city} 作为对照组。",
            f"每周复盘激活率与参与率（当前 {activation:.1f}% / {participation:.1f}%），不达标即调整话术与陈列。",
        ]
    elif mode == "over":
        fallback_judgement = (
            f"{product.product_name} 当前属于“呈现高于内在”的过度包装风险型，应先校准价值锚点，避免靠话术拉动短期转化。"
        )
        fallback_summary = (
            f"{product.product_name} 呈现价值 {presented:.1f} 分高于内在价值 {intrinsic:.1f} 分，且市场验证 {validation:.1f} 分，"
            f"建议降低夸张表达，先回到产品本体和产地可信证据，在 {top_city}、{second_city} 做稳态验证。{evidence_hint}"
        )
        fallback_actions = [
            f"下调过度营销表述，把 {product.product_name} 的价值表述收敛到可验证的产品与产地事实。",
            origin_action,
            f"优先在 {top_city}、{second_city} 进行低风险试销，观察真实复购与退货反馈后再扩城。",
            "将短期转化目标改为“复购率+客诉率”双指标，避免只看首单成交。",
        ]
    elif mode == "matched":
        fallback_judgement = f"{product.product_name} 当前处于“价值-呈现”相对匹配区间，重点应放在渠道放大和节奏控制。"
        fallback_summary = (
            f"{product.product_name} 适配度 {fit_score:.1f} 分，建议保持当前主叙事不大改，"
            f"在 {top_city} 继续做深，在 {second_city}、{third_city} 做低成本复制，并持续监控验证效率。"
        )
        fallback_actions = [
            f"保持 {product.product_name} 现有主卖点，避免频繁改定位导致认知漂移。",
            origin_action,
            f"按“{top_city} 深耕、{second_city}/{third_city} 复制”的节奏推进渠道扩展。",
            "设置月度复盘机制，按转化、复购、异常反馈三项共同判断是否扩量。",
        ]
    else:
        fallback_judgement = f"{product.product_name} 当前处于流通观察期，应优先跑通“区域-渠道-表达”闭环，而不是盲目扩量。"
        fallback_summary = (
            f"{product.product_name} 当前处于验证爬坡阶段，建议先在 {top_city} 建立稳定样板，再向 {second_city}、{third_city} 扩展，"
            f"通过小步试错明确最有效的产品表达与渠道组合。{evidence_hint}"
        )
        fallback_actions = [
            f"围绕 {product.product_name} 建立单一清晰叙事，先确保一个场景讲透再扩展。",
            origin_action,
            f"采用“1 个样板城市 + 2 个验证城市”结构（{top_city} / {second_city} / {third_city}）推进投放。",
            "设置止损阈值，连续两周期指标无改善则回滚方案并重新定义人群与渠道。",
        ]
    ...
    analysis.core_judgement = _normalize_core_judgement_v2(analysis.core_judgement, product, mode)
    ...
    if not analysis.pricing_packaging_advice:
        analysis.pricing_packaging_advice = (
            f"{product.product_name} 建议围绕 {product.price_band} 保持价格带稳定，包装只做支持性优化："
            "提升信息清晰度与使用便利性，不通过堆叠物料制造虚高感。"
        )
    if not analysis.channel_advice:
        analysis.channel_advice = (
            f"渠道上建议先做 {top_city} 样板，再拓展到 {second_city}、{third_city}。"
            f"优先使用 {product.channel} 的高匹配场景，先验证单店模型再放大。"
        )
    ...
    if not analysis.risk_warning:
        analysis.risk_warning = (
            f"当前激活率 {activation:.1f}%、参与率 {participation:.1f}% 仍需提升。"
            "若继续放大投放但不先修复渠道匹配与表达一致性，容易造成费用增加而转化不升。"
        )
...
def _default_core_judgement(product, mode: str) -> str:
    name = str(product.product_name)
    region = str(product.region_name)
    if mode == "under":
        return f"{name}在{region}具备产地与本体优势，当前市场表达偏保守，建议先做适度升级验证。"
    if mode == "over":
        return f"{name}在{region}当前呈现强于本体支撑，建议收敛营销话术并回归可验证价值。"
    if mode == "matched":
        return f"{name}在{region}的定位与市场承接基本匹配，可按现有策略稳步放大。"
    return f"{name}在{region}仍处于验证阶段，建议先跑通区域与渠道闭环再扩量。"


def _normalize_core_judgement(text: str, product, mode: str) -> str:
    cleaned = _clean_block_text(text)
    if not cleaned:
        return _default_core_judgement(product, mode)

    # Remove numeric/technical fragments that make the headline too dense.
    cleaned = re.sub(r"（[^）]*\d[^）]*）", "", cleaned)
    cleaned = re.sub(r"\([^)]*\d[^)]*\)", "", cleaned)
    cleaned = re.sub(r"(GI|ROI|GMV|CPC|CTR|CVR)\s*强度?\s*[（(]?[0-9.]+[）)]?", "", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\s+", " ", cleaned).strip()

    # Keep only the first sentence-level conclusion.
    if "：" in cleaned:
        left, right = cleaned.split("：", 1)
        cleaned = left if len(left) >= 10 else right
    first_sentence = re.split(r"[。！？!?]", cleaned)[0].strip()
    first_sentence = re.sub(r"[，,;；:：]\s*$", "", first_sentence)

    # Force concise phrasing for the bold headline.
    if len(first_sentence) > 56:
        segments = [seg.strip() for seg in re.split(r"[，,、]", first_sentence) if seg.strip()]
        clipped = ""
        for seg in segments:
            candidate = f"{clipped}，{seg}" if clipped else seg
            if len(candidate) <= 56:
                clipped = candidate
            else:
                break
        first_sentence = clipped or segments[0] if segments else first_sentence[:56]

    name = str(product.product_name)
    region = str(product.region_name)
    if name not in first_sentence or (region and region not in first_sentence):
        return _default_core_judgement(product, mode)
...
def _default_core_judgement_v2(product, mode: str) -> str:
    name = str(product.product_name)
    region = str(product.region_name)
    if mode == "under":
        return f"{name}在{region}具备基础优势，当前更需要把价值讲清楚并完成小范围验证。"
    if mode == "over":
        return f"{name}在{region}当前呈现略快于价值支撑，建议先收敛表达并补强可验证证据。"
    if mode == "matched":
        return f"{name}在{region}的定位与市场承接基本匹配，可在现有策略上稳步放大。"
    return f"{name}在{region}仍处于验证阶段，建议先跑通区域与渠道闭环再扩量。"


def _normalize_core_judgement_v2(text: str, product, mode: str) -> str:
    cleaned = _clean_block_text(text)
    if not cleaned:
        return _default_core_judgement_v2(product, mode)

    name = str(product.product_name)
    region = str(product.region_name)

    # Prefer model meaning: keep first conclusion sentence, not full detail chain.
    sentence = cleaned
    if "：" in sentence:
        left, right = sentence.split("：", 1)
        sentence = right if len(left) <= 12 else sentence
    sentence = re.split(r"[。！？!?]", sentence)[0].strip()

    # Remove numeric/technical fragments from headline-level sentence.
    sentence = re.sub(r"（[^）]*\d[^）]*）", "", sentence)
    sentence = re.sub(r"\([^)]*\d[^)]*\)", "", sentence)
    sentence = re.sub(r"(GI|ROI|GMV|CPC|CTR|CVR)\s*强度?\s*[（(]?[0-9.]+[）)]?", "", sentence, flags=re.IGNORECASE)
    sentence = re.sub(r"\s+", "", sentence)
    sentence = re.sub(rf"^{re.escape(name)}{re.escape(region)}", name, sentence)
    sentence = re.sub(rf"^{re.escape(name)}该", f"{name}", sentence)
    sentence = re.sub(rf"^{re.escape(name)}这款", f"{name}", sentence)

    # Keep concise but avoid hard templating.
    if len(sentence) > 54:
        ...
        for part in parts:
            ...
            if len(candidate) <= 50:
                ...
                break
        sentence = "，".join(kept) if kept else sentence[:50]

    sentence = re.sub(r"[，,；;:：]\s*$", "", sentence).strip()
    sentence = re.sub(r"^(该产品|这款产品|该商品)", name, sentence)
    sentence = sentence.replace("，地理标志", "和地理标志")
    sentence = sentence.replace("，，", "，")

    if name not in sentence:
        sentence = f"{name}{sentence}" if sentence else f"{name}在{region}仍有提升空间"
    if region and region not in sentence:
        sentence = f"{sentence}，产地为{region}"
    if region and f"{name}{region}" in sentence:
        sentence = sentence.replace(f"{name}{region}", f"{name}在{region}", 1)
